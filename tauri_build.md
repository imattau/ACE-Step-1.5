...
WebKit downloads initiated by the existing Gradio Save controls are intercepted by
the Tauri shell and routed through `GtkFileChooserNative`. Inside Flatpak this uses
the save-file portal, returning a document-portal grant for the chosen destination.
The temporary `xdg-download` permission is therefore no longer required. External
model directories remain unsupported; model data stays in the private application
data directory.
